...
const profile = await a.client.from('profiles').upsert({ id: a.user.id, display_name: 'Integration A', interface_language: 'en' }).select('id').single()
...
const bProfile = await b.client.from('profiles').upsert({ id: b.user.id, display_name: 'Integration B', interface_language: 'en' }).select('id').single()
