-- Keep Friends standings limited to accepted friends and Group League details
-- limited to current Group members. Migration 0028 and prior history stay put.
...
begin
  ...
  if p_league_type = 'group' and public.group_role(p_group_id) = '' then
  ...
  select jsonb_build_object(
    ...
    'participantCount', case when p_league_type = 'friends' then (
      select count(*) from public.league_participants p
      where p.season_id = s.id and p.left_at is null
  ...
  from (
    ...
    where p.season_id = v_season_id and p.left_at is null
      and (p_league_type <> 'friends' or p.user_id = auth.uid() or exists (
        select 1 from public.friendships f
        where f.status = 'accepted'
          and ((f.requester_id = auth.uid() and f.addressee_id = p.user_id)
            or (f.addressee_id = auth.uid() and f.requester_id = p.user_id))
