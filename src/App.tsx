...
import { createPuterProvider } from './lib/ai/puterProvider'
import type { AiFeedbackType, AiModel, AiProvider, AiStartConfig, ResolvedOpponent } from './lib/ai/types'
...
type AiMode = 'MOCK_AI' | 'configured AI' | 'MOCK_AI (offline fallback)'
...
function BetaNotice({ aiMode }: { aiMode: AiMode }) {
  return <div className="beta-notice" role="note"><Icon name="info" size={14} /> Private beta: {aiMode === 'configured AI' ? 'AI is connected server-side.' : 'mock AI is active; replies and scores are development simulations, not expert advice.'} <button type="button" className="text-link" onClick={() => window.dispatchEvent(new Event('sideshift-debate-entry'))}>Choose how to debate</button></div>
}
...
function TakeCard({ take, onBegin, onChooseDebate, featured = false, language }: { take: Take; onBegin: BeginHandler; onChooseDebate?: (take: Take) => void; featured?: boolean; language: Language }) {
  ...
  const symbol = take.category === 'Football' ? 'â—‰' : take.category.includes('Gaming') ? 'â—ˆ' : 'âœ¦'
...
function RecentShift({ result, language }: { result: ResultData; language: Language }) {
  ...
  return <article className="recent-card card-surface"><span className="recent-icon lavender"><Icon name="spark" size={18} /></span><div><h3>{text.statement}</h3><p>{result.mode} Â· {new Date(result.completedAt).toLocaleDateString(language === 'de' ? 'de-DE' : 'en-GB')}</p></div><strong>{result.score}<small>/100</small></strong><Icon name="chevron" size={17} /></article>
}
...
function Debate({ activeTake, language, mode, step, setStep, stance, setStance, confidence, setConfidence, postStance, setPostStance, understanding, setUnderstanding, responses, setResponses, opponentMessages, setOpponentMessages, onModeChange, onComplete, onExit, onNotify, onReport, aiMode, online }: { activeTake: Take; language: Language; mode: Mode; step: number; setStep: (step: number) => void; stance: Stance; setStance: (stance: Stance) => void; confidence: number; setConfidence: (value: number) => void; postStance: Stance; setPostStance: (stance: Stance) => void; understanding: string; setUnderstanding: (value: string) => void; responses: Record<number, string>; setResponses: React.Dispatch<React.SetStateAction<Record<number, string>>>; opponentMessages: Record<number, string>; setOpponentMessages: React.Dispatch<React.SetStateAction<Record<number, string>>>; onModeChange: (mode: Mode) => void; onComplete: () => Promise<void>; onExit: () => void; onNotify: (message: string) => void; onReport: (payload: ReportInput) => Promise<void>; aiMode: AiMode; online?: boolean }) {
  ...
  async function saveResponse() {
    ...
    try {
      ...
      setOpponentMessages(current => ({ ...current, [step]: `${output.response}\n\nQuestion: ${output.question}` }))
      clearArgumentDraft(draftKey)
      setStep(step + 1)
    } catch {
      const fallback = createMockOpponent(activeTake, assignedSide, step, trimmed, language)
      setOpponentMessages(current => ({ ...current, [step]: `${fallback.response}\n\nQuestion: ${fallback.question}` }))
      setStep(step + 1)
...
function Results({ result, language, onBegin, onClash, onNotify }: { result: ResultData; language: Language; onBegin: BeginHandler; onClash: () => void; onNotify: (message: string) => void }) {
  ...
  async function shareResult() {
    ...
    onNotify(await copyText(shareData.text) ? 'Caption copied â€” the share sheet is unavailable in this browser.' : 'Sharing is unavailable in this browser.')
  }
...
function ChallengeRecipient({ token, repository, userId, online }: { token: string; repository: AppRepository; userId: string; online?: boolean }) {
  ...
  if (loading && !challenge) return <div className="onboarding-page"><div className="created-challenge"><span className="eyebrow">FRIEND CLASH</span><h1>Loading challengeâ€¦</h1></div></div>
...
function App() {
  ...
  const [aiMode, setAiMode] = useState<AiMode>('MOCK_AI (offline fallback)')
  const aiProvider = useMemo<AiProvider>(() => import.meta.env.VITE_AI_MOCK === 'true' ? createMockAiProvider({ streamDelayMs: 1 }) : createPuterProvider(), [])
  ...
  useEffect(() => {
    ...
    void repository.saveDebate(userId, debate).catch(caught => setDataError(caught instanceof Error ? caught.message : 'Private debate data could not be saved.'))
  ...
  useEffect(() => {
    ...
    apiFetch<{ aiMode: string; persistence: string }>('/api/health').then(value => {
      setAiMode(livePuter ? 'configured AI' : value.aiMode === 'configured AI' ? 'configured AI' : 'MOCK_AI')
      if (repository && value.persistence && value.persistence !== repository.backend) setDataError(`Frontend and API backends disagree: ${repository.backend} vs ${value.persistence}.`)
    }).catch(() => setAiMode(livePuter ? 'configured AI' : 'MOCK_AI (offline fallback)'))
  }, [repository])
  ...
  async function beginDebate(mode: Mode, take = activeTake) {
    ...
    try {
      await repository.saveDebate(userId, snapshot)
  ...
  async function completeDebate() {
    ...
    try { judge = await apiFetch<ReturnType<typeof calculateMockScore>>('/api/ai/judge', { method: 'POST', body: JSON.stringify({ transcript, language }) }) } catch { judge = calculateMockScore(responses, understanding, movementBetween(stance, postStance)); setAiMode('MOCK_AI (offline fallback)'); notify('Scoring service unavailable; a bounded development score was used.') }
    const now = new Date().toISOString()
  ...
  if (auth.loading) return <BackendGate title="Connectingâ€¦" message="Establishing your private SideShift session." />
  ...
  if (hydratedUserId !== userId) return <BackendGate title="Loading your spaceâ€¦" message="Restoring your private debates and preferences." />
