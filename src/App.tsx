...
function MobileOnboarding({ userId, language, initialProgress, onProgress, onComplete, onLanguageChange, online }: OnboardingProps) {
  ...
  const goals = [['reasoning', t('onboarding.goalReasoning')], ['school', t('onboarding.goalSchool')], ['friends', t('onboarding.goalFriends')], ['perspectives', t('onboarding.goalPerspectives')], ['fun', t('onboarding.goalFun')]] as const
  const modes = [[t('onboarding.modeBasicTitle'), t('onboarding.modeBasicBody'), 'spark'], [t('onboarding.modePersonTitle'), t('onboarding.modePersonBody'), 'person'], [t('onboarding.modeTeamTitle'), t('onboarding.modeTeamBody'), 'users']] as const
  return <div className="onboarding-page onboarding-mobile"><div className="onboarding-top"><Logo /><span className="onboarding-meta"><Icon name="lock" size={14} /> {t('onboarding.privateByDefault')}</span></div><div className="onboarding-progress" role="progressbar" aria-valuemin={1} aria-valuemax={4} aria-valuenow={step + 1} aria-label={t('onboarding.progress', { current: step + 1, total: 4 })}>{[0, 1, 2, 3].map(item => <span key={item} className={item <= step ? 'active' : ''} />)}</div><main className="onboarding-mobile-main" aria-live="polite"><Tag tone="coral">{t('onboarding.eyebrow')}</Tag>{step === 0 && <section className="onboarding-mobile-panel"><div className="onboarding-visual" aria-hidden="true"><span>↗</span><i /><b /></div><span className="eyebrow">01 · {t('onboarding.privateByDefault')}</span><h1>{t('onboarding.welcomeTitle')}</h1><p>{t('onboarding.welcomeBody')}</p><label className="field-label" htmlFor="onboarding-language">{t('settings.interfaceLanguage')}</label><select id="onboarding-language" className="settings-select" value={language} onChange={event => { const next = event.target.value as Language; persistLanguage(next); onLanguageChange?.(next); window.dispatchEvent(new Event('sideshift-onboarding-language')) }}>{supportedLanguages.map(item => <option value={item} key={item}>{localeLabels[item]}</option>)}</select><Button className="full-width" icon="arrow" onClick={() => void move(1)}>{t('common.continue')}</Button></section>}{step === 1 && <section className="onboarding-mobile-panel"><span className="eyebrow">02 · {t('onboarding.chooseModeTitle')}</span><h1>{t('onboarding.chooseModeTitle')}</h1><p>{t('onboarding.chooseModeBody')}</p><div className="onboarding-mode-list">{modes.map(([title, body, icon]) => <article key={title}><span className="onboarding-mode-icon"><Icon name={icon as IconName} size={19} /></span><div><h2>{title}</h2><p>{body}</p></div></article>)}</div><Button className="full-width" icon="arrow" onClick={() => void move(2)}>{t('common.continue')}</Button></section>}{step === 2 && <section className="onboarding-mobile-panel"><span className="eyebrow">03 · SIDESWITCH</span><h1>{t('onboarding.switchTitle')}</h1><p>{t('onboarding.switchBody')}</p><div className="onboarding-example"><span className="eyebrow">SIDE A → SIDE B</span><strong>{t('onboarding.switchExample')}</strong></div><p className="onboarding-benefits">{t('onboarding.switchBenefits')}</p><Button className="full-width" icon="arrow" onClick={() => void move(3)}>{t('common.continue')}</Button></section>}{step === 3 && <section className="onboarding-mobile-panel"><span className="eyebrow">04 · {t('onboarding.personalizeTitle')}</span><h1>{t('onboarding.personalizeTitle')}</h1><p>{t('onboarding.personalizeBody')}</p><label className="field-label" htmlFor="onboarding-name">{t('onboarding.displayName')}</label><input id="onboarding-name" className="text-input" value={name} onChange={event => { setName(event.target.value); setError('') }} maxLength={24} autoComplete="nickname" placeholder={t('onboarding.yourName')} /><span className="field-label">{t('onboarding.arenas')}</span><div className="interest-grid">{interestOptions.map(interest => <button type="button" key={interest} className={`interest-chip ${selected.includes(interest) ? 'selected' : ''}`} aria-pressed={selected.includes(interest)} onClick={() => setSelected(current => current.includes(interest) ? current.filter(item => item !== interest) : [...current, interest])}>{localizeInterest(interest, language)}</button>)}</div><span className="field-label">{t('onboarding.goal')}</span><div className="interest-grid">{goals.map(([value, label]) => <button type="button" key={value} className={`interest-chip ${goal === value ? 'selected' : ''}`} aria-pressed={goal === value} onClick={() => setGoal(value)}>{label}</button>)}</div><Button className="full-width" icon="arrow" onClick={() => void finish()} disabled={busy}>{busy ? t('common.loading') : t('onboarding.startMyFirstDebate')}</Button></section>}{error && <p className="form-error" role="alert">{error}</p>}<div className="onboarding-mobile-actions">{step > 0 && <button type="button" className="text-link" onClick={() => void move(step - 1)} disabled={busy}><Icon name="arrow" size={14} /> {t('common.back')}</button>}<button type="button" className="text-link" onClick={() => void skip()} disabled={busy}>{t('onboarding.skipShort')}</button></div></main></div>
}
