...
import { getInitialLanguage, greetingKey, localizeInterest, localeLabels, persistLanguage, supportedLanguages, translate, useTranslations } from './i18n'
...
function Onboarding({ language, onComplete, online }: { language: Language; onComplete: (name: string, interests: string[]) => Promise<void>; online?: boolean }) {
  const t = useTranslations(language)
...
function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([repository.loadProfile(userId), repository.loadPreferences(userId), repository.loadDebate(userId), repository.loadResult(userId), repository.loadHistory(userId), repository.loadTeamSession(userId)]).then(([profile, preferences, debate, result, storedHistory, storedTeamSession]) => {
      ...
      const nextLanguage = profile?.interfaceLanguage || preferences?.debateLanguages[0] || getInitialLanguage()
  ...
  if (!hasOnboarded) return <Onboarding language={language} onComplete={completeOnboarding} />
