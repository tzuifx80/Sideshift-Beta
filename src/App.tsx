...
function OnboardingV3({ userId, language, onComplete, onLanguageChange, onFirstAction, online }: { userId: string; language: Language; onComplete: (name: string, interests: string[]) => Promise<void>; onLanguageChange?: (language: Language) => void; onFirstAction?: (action: 'person' | 'team') => void; online?: boolean }) {
  const t = useTranslations(language)
  ...
  const [selected, setSelected] = useState(savedProgress.selected?.length ? savedProgress.selected : ['Politics and Democracy', 'Football', 'AI and Technology'])
  const [goal, setGoal] = useState(savedProgress.goal || 'reasoning')
  ...
  function saveStep(next: number) { setStep(next); window.localStorage.setItem(key, serializeOnboardingProgress({ step: next, name, selected, goal })) }
  async function finish(action: 'basic' | 'person' | 'team' | 'skip' = 'skip') {
    ...
    try { await onComplete(name.trim(), selected); window.localStorage.removeItem(key); if (action === 'basic') window.dispatchEvent(new Event('sideshift-basic-entry')); else if (action !== 'skip') onFirstAction?.(action) } catch (caught) { setError(caught instanceof Error ? caught.message : t('onboarding.saveError')) } finally { setBusy(false) }
  }
...
function Onboarding({ userId, language, onComplete, onLanguageChange, onFirstAction, online }: { userId: string; language: Language; onComplete: (name: string, interests: string[]) => Promise<void>; onLanguageChange?: (language: Language) => void; onFirstAction?: (action: 'person' | 'team') => void; online?: boolean }) {
  return <OnboardingV3 userId={userId} language={language} onComplete={onComplete} onLanguageChange={onLanguageChange} onFirstAction={onFirstAction} online={online} />
  ...
  async function submit() {
    ...
    if (selected.length < 3) return setError(t('onboarding.interestsTooFew'))
    setBusy(true)
    try { await onComplete(trimmed, selected) } catch (caught) { setError(caught instanceof Error ? caught.message : t('onboarding.saveError')) } finally { setBusy(false) }
  }
...
function AppShellV2({ children, screen, name, historyCount, onNavigate, onLanguage, language, aiMode, onNotify, online, onDelete, hasUnsavedDraft }: { children: ReactNode; screen: Screen; name: string; historyCount: number; onNavigate: (screen: Screen) => void; onLanguage: () => void; language: Language; aiMode: AiMode; onNotify: (message: string) => void; online?: boolean; onDelete?: () => void; hasUnsavedDraft?: boolean }) {
  const t = useTranslations(language)
  const navItems = [{ id: 'home' as Screen, label: t('nav.home'), icon: 'home' as IconName }, { id: 'explore' as Screen, label: t('nav.explore'), icon: 'layers' as IconName }, { id: 'groups' as Screen, label: t('nav.groups'), icon: 'users' as IconName }, { id: 'profile' as Screen, label: t('nav.profile'), icon: 'person' as IconName }]
...
function SettingsScreen(props: { profile: UserProfile; preferences: UserPreferences; aiMode: AiMode; backendLabel: string; onSaveProfile: (value: UserProfile) => Promise<void>; onSavePreferences: (value: UserPreferences) => Promise<void>; onDelete: () => void; onBack: () => void; onNotify: (message: string) => void; onSubmitFeedback: (payload: BetaFeedbackInput) => Promise<void> }) {
  return <LocalizedSettingsScreen {...props} />
}
...
function App() {
  ...
  if (!hasOnboarded || showOnboarding) return <Onboarding userId={userId} language={language} onComplete={async (...args) => { await completeOnboarding(...args); setShowOnboarding(false) }} onLanguageChange={setLanguage} onFirstAction={action => { setActiveTake(activeTake); if (action === 'person') setScreen('clash'); else openTeamDebate() }} />
