...
type Screen = 'home' | 'explore' | 'friends' | 'groups' | 'team' | 'debateChoice' | 'debate' | 'results' | 'clash' | 'profile' | 'settings' | 'aiSetup' | 'aiDebate' | 'aiResults'
...
function defaultProfile(id: string): UserProfile {
  return { id, displayName: null, bio: null, avatarPreset: 'orbit', interfaceLanguage: 'en', challengeShowName: false, shareRealStance: false, publicProfileKey: null, handle: null, friendCode: null, avatarPath: null, profileAccent: 'coral', profileVisibility: 'friends', avatarVisibility: 'private', visibleStats: { debates: true, sideSwitches: true, constructive: true, argumentDna: false } }
}
...
function Onboarding({ userId, language, initialProgress, onProgress, onComplete, onLanguageChange, onFirstAction, online }: { userId: string; language: Language; initialProgress?: { stage: number; goal: UserPreferences['onboardingGoal']; name: string; selected: string[] }; onProgress?: (stage: number, goal: UserPreferences['onboardingGoal'], name: string, selected: string[]) => Promise<void>; onComplete: (name: string, interests: string[], goal: UserPreferences['onboardingGoal']) => Promise<void>; onLanguageChange?: (language: Language) => void; onFirstAction?: (action: 'person' | 'team') => void; online?: boolean }) {
  return <OnboardingV3 userId={userId} language={language} initialProgress={initialProgress} onProgress={onProgress} onComplete={onComplete} onLanguageChange={onLanguageChange} onFirstAction={onFirstAction} online={online} />
  /* Legacy markup below remains intentionally unreachable until removed after the Phase 3 browser pass. */
  const t = useTranslations(language)
...
function ProfileSocialPanel({ profile, userId, repository, onProfile, onNotify }: { profile: UserProfile; userId: string; repository: AppRepository; onProfile: (value: UserProfile) => Promise<void>; onNotify: (message: string) => void }) {
  ...
  async function upload(file: File) { setBusy(true); try { const { processAvatarFile } = await import('./avatar'); const path = await repository.uploadAvatar(userId, await processAvatarFile(file), 'image/webp'); const next = { ...draft, avatarPath: path }; setDraft(next); await onProfile(next); onNotify(t('settings.saved')) } catch (caught) { onNotify(caught instanceof Error ? caught.message : t('friends.unavailable')) } finally { setBusy(false) } }
  async function remove() { setBusy(true); try { await repository.removeAvatar(userId); const next = { ...draft, avatarPath: null }; setDraft(next); await onProfile(next); onNotify(t('settings.saved')) } catch (caught) { onNotify(caught instanceof Error ? caught.message : t('friends.unavailable')) } finally { setBusy(false) } }
  return <section className="settings-section card-surface"><div className="settings-section-heading"><div><span className="eyebrow">{t('friends.eyebrow')}</span><h2>{t('friends.photo')}</h2></div><Icon name="shield" size={21} /></div><p className="field-help">{t('friends.photoHelp')}</p><div className="profile-photo-actions"><Button variant="secondary" onClick={() => fileRef.current?.click()} disabled={busy}>{t('friends.photo')}</Button>{draft.avatarPath && <button type="button" className="text-link" onClick={() => void remove()} disabled={busy}>{t('friends.removePhoto')}</button>}<input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" hidden onChange={event => { const file = event.target.files?.[0]; event.currentTarget.value = ''; if (file) void upload(file) }} /></div><label className="field-label">{t('friends.handle')}<input className="text-input" maxLength={24} value={draft.handle || ''} onChange={event => setDraft(current => ({ ...current, handle: event.target.value }))} placeholder="@handle" /></label><label className="field-label">{t('friends.visibility')}<select className="settings-select" value={draft.profileVisibility} onChange={event => setDraft(current => ({ ...current, profileVisibility: event.target.value as UserProfile['profileVisibility'] }))}><option value="friends">{t('friends.friendsOnly')}</option><option value="shared_groups">{t('friends.sharedGroups')}</option><option value="private">{t('friends.private')}</option></select></label><span className="field-label">{t('friends.stats')}</span>{(['debates', 'sideSwitches', 'constructive', 'argumentDna'] as const).map(stat => <label className="toggle-row" key={stat}><input type="checkbox" checked={draft.visibleStats[stat]} onChange={event => setDraft(current => ({ ...current, visibleStats: { ...current.visibleStats, [stat]: event.target.checked } }))} /><span>{({ debates: t('friends.statDebates'), sideSwitches: t('friends.statSideSwitches'), constructive: t('friends.statConstructive'), argumentDna: t('friends.statArgumentDna') } as const)[stat]}</span></label>)}<Button variant="dark" onClick={() => void save()} disabled={busy}>{t('common.save')}</Button></section>
}
...
function App() {
  ...
  async function completeOnboarding(name: string, nextInterests: string[], onboardingGoal: UserPreferences['onboardingGoal']) {
    ...
    const nextPreferences = normalizePreferences({ ...preferencesData, userId, topicPreferences: nextInterests, debateLanguages: [language], onboardingCompleted: true, onboardingStage: 3, onboardingGoal, onboardingDismissed: false })
  ...
  async function saveProfileSettings(next: UserProfile) {
    ...
    setProfileData(normalized); setUserName(normalized.displayName || ''); setLanguage(normalized.interfaceLanguage)
  }
