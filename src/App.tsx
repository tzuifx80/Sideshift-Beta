...
function ProfileSocialPanel({ profile, userId, repository, onProfile, onNotify }: { profile: UserProfile; userId: string; repository: AppRepository; onProfile: (value: UserProfile) => Promise<void>; onNotify: (message: string) => void }) {
  ...
  async function upload(file: Blob) { setBusy(true); try { const path = await repository.uploadAvatar(userId, file, 'image/webp'); const next = { ...draft, avatarPath: path }; setDraft(next); await onProfile(next); onNotify(t('settings.saved')) } catch (caught) { onNotify(caught instanceof Error ? caught.message : t('friends.unavailable')) } finally { setBusy(false) } }
