...
export function normalizeAiError(error: unknown): AiProviderError {
  ...
  if (code.includes('server_unreachable') || /sideShift server.*(reach|unreachable)|api url.*missing/i.test(message)) return new AiProviderError('server_unreachable', 'The SideShift server could not be reached. Check the Android development API URL and try again.', true)
