...
export function normalizeAiError(error: unknown): AiProviderError {
  ...
  if (code.includes('server_unreachable') || /sideShift server.*(reach|unreachable)|api url.*missing/i.test(message)) return new AiProviderError('server_unreachable', 'The SideShift server could not be reached. Check your connection and API configuration, then try again.', true)
  ...
  if (/rate.?limit/i.test(message)) return new AiProviderError('rate_limited', 'SideShift AI is rate limited. Try again shortly.', true)
  if (/temporarily unavailable|provider unavailable/i.test(message)) return new AiProviderError('provider_unavailable', 'SideShift AI is temporarily unavailable. Keep Connect Puter available.')
  ...
  if (/network|fetch|offline|failed to load/i.test(message)) return new AiProviderError('temporary_failure', 'The SideShift server could not be reached. Check your connection and retry.', true)
