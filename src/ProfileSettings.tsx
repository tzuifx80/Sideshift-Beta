...
import type { BetaFeedbackInput } from './data/repository'
import type { AiRuntimeStatus } from './lib/ai/types'
import { aiRuntimeLabel } from './lib/ai/runtimeStatus'
import { localeLabels, supportedLanguages } from './i18n'
...
type ProfileSettingsProps = {
  ...
  onSubmitBetaFeedback: (payload: BetaFeedbackInput) => Promise<void>
  backendLabel: string
  aiMode: AiRuntimeStatus
...
export function ProfileSettings({ profile, preferences, user, userId, repository, language, backendLabel, aiMode, initialSection = 'hub', onSaveProfile, onSavePreferences, onBack, onNotify, onDelete, onSignOut, onRequestSecureAccountOtp, onVerifySecureAccountOtp, onOpenOnboarding, onOpenProfile, onSubmitBetaFeedback, hasUnsavedDraft = false }: ProfileSettingsProps) {
  ...
  async function confirmSignOut() {
    logoutDiagnostic('confirmation_accepted')
    await signOut()
  }
  ...
  return <div className="page profile-settings-page"><div className="page-heading compact-page-heading"><div><button type="button" className="back-link" onClick={goBack}><Icon name="arrow" size={15} /> {t('common.back')}</button><span className="eyebrow">{t('settings.eyebrow')}</span><h1>{title}<span className="heading-period">.</span></h1></div>{(section === 'profile' || section === 'debate' || section === 'appearance') && <Button variant="dark" onClick={() => void (section === 'profile' ? (validateLinks() ? saveProfile() : Promise.resolve()) : section === 'appearance' ? savePreferences() : savePreferences())} disabled={busy}>{busy ? t('common.saving') : t('common.save')}</Button>}</div>
    ...
    {section === 'help' && <><section className="settings-section card-surface"><Button variant="secondary" onClick={onOpenOnboarding}>{t('settings.guide')}</Button><p className="field-help">{t('profileSettings.helpBody')}</p><p className="field-help">{t('ai.reliableHelpBody')}</p><p className="field-help">{t('profileSettings.supportEmail')}</p><div className="beta-facts"><span><strong>{t('settings.backend')}</strong>{backendLabel}</span><span><strong>{t('settings.aiStatus')}</strong>{aiRuntimeLabel(aiMode)}</span><span><strong>{t('settings.version')}</strong>{import.meta.env.VITE_APP_VERSION || '0.1.0-beta'}</span></div><div className="settings-links"><a href="/privacy">{t('shell.privacy')}</a><a href="/terms">{t('shell.betaTerms')}</a><a href="/community">{t('shell.communityRules')}</a></div></section><BetaFeedbackForm language={language} surface="settings" screen="settings" onSubmit={onSubmitBetaFeedback} /></>}
