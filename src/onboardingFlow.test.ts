...
describe('mobile onboarding flow', () => {
  it('uses the phone-first story order', () => {
    expect(onboardingStepOrder).toEqual(['welcome', 'sideswitch', 'personalize'])
  })

  it('resumes a saved stage without treating completion as a new onboarding run', () => {
    expect(parseOnboardingProgress(JSON.stringify({ step: 2, name: 'Alex', selected: ['Football'], goal: 'fun' }))).toMatchObject({ step: 1, name: 'Alex', goal: 'fun' })
    expect(parseOnboardingProgress(JSON.stringify({ step: 3 }))).toMatchObject({ step: 2 })
    expect(parseOnboardingProgress(JSON.stringify({ step: 9 }))).toMatchObject({ step: 2 })
  })
