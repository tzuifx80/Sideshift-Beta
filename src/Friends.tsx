...
function ProfilePreviewCard({ repository, userId, profile, fallback, children, onOpenProfile }: { repository: AppRepository; userId: string; profile: ProfilePreview | null; fallback: string; children?: ReactNode; onOpenProfile?: (profileKey: string) => void }) {
...
export function Friends({ userId, language, repository, profile, onProfile, onOpenProfile, online, onNotify }: FriendsProps) {
  ...
  return <ProfileNavigationContext.Provider value={openProfile}><div className="page friends-page"><div className="page-heading"><div><span className="eyebrow">{t('friends.eyebrow')}</span><h1>{t('friends.title')}<span className="heading-period">.</span></h1><p className="muted">{t('friends.body')}</p></div><Tag tone="coral"><Icon name="lock" size={13} /> {t('common.private')}</Tag></div>
    <section className="friends-profile card-surface"><div><span className="eyebrow">{t('friends.photo')}</span><p>{t('friends.photoHelp')}</p><AvatarPhotoPicker language={language} hasAvatar={Boolean(profile.avatarPath)} busy={busy} onUpload={uploadPhoto} onRemove={removePhoto} onNotify={onNotify} /></div><div className="friend-code-box"><span className="eyebrow">{t('friends.code')}</span><strong>{profile.friendCode || '—'}</strong><div><button type="button" className="text-link" onClick={() => void copyCode()}>{t('friends.copyCode')}</button><button type="button" className="text-link" onClick={() => void regenerateCode()}>{t('friends.codeRegenerate')}</button></div></div></section>
    <section className="settings-section card-surface"><div className="settings-section-heading"><div><span className="eyebrow">{t('friends.add')}</span><h2>{t('friends.lookup')}</h2></div><Icon name="search" size={21} /></div><p className="field-help">{t('friends.lookupHint')}</p><div className="friends-lookup-row"><select className="settings-select" value={queryKind} onChange={event => setQueryKind(event.target.value as 'handle' | 'code')}><option value="handle">{t('friends.handle')}</option><option value="code">{t('friends.code')}</option></select><input className="text-input" value={query} onChange={event => setQuery(event.target.value)} placeholder={queryKind === 'handle' ? '@handle' : 'SS-XXXXXXXXXX'} /><Button variant="dark" onClick={() => void lookup()} disabled={!query.trim() || busy}>{t('friends.lookup')}</Button></div>{result ? <ProfilePreviewCard onOpenProfile={openProfile} repository={repository} userId={userId} profile={result} fallback={t('friends.member')}><Button variant="secondary" onClick={() => void run(async () => { await repository.sendFriendRequest(userId, result.profileKey); setResult(null) })}>{t('friends.request')}</Button></ProfilePreviewCard> : query && <p className="muted">{t('friends.noResults')}</p>}</section>
    ...
    {challenges.length > 0 && <section className="settings-section card-surface"><div className="settings-section-heading"><h2>{t('friends.challenges')}</h2></div>{challenges.map(item => <div className="friend-challenge" key={item.id}>{(item.direction === 'incoming' ? item.creator : item.recipient) && <ProfilePreviewCard repository={repository} userId={userId} profile={item.direction === 'incoming' ? item.creator : item.recipient} fallback={t('friends.member')} onOpenProfile={openProfile} />}<div><Tag tone={item.direction === 'incoming' ? 'yellow' : 'mint'}>{item.direction === 'incoming' ? t('friends.answer') : item.status}</Tag><strong>{getTake(item.takeId) ? getTake(item.takeId).statement : item.takeId}</strong><p>{item.argument}</p></div>{item.direction === 'incoming' && item.status === 'open' && <Button variant="secondary" onClick={() => setAnswerId(item.id)}>{t('friends.answer')}</Button>}{answerId === item.id && <div className="friend-answer"><textarea className="settings-textarea" value={answer} onChange={event => setAnswer(event.target.value)} placeholder={t('friends.answerPlaceholder')} /><Button onClick={() => void run(async () => { await repository.completeFriendChallenge(userId, item.id, answer); setAnswerId(null); setAnswer('') })} disabled={answer.trim().length < 12}>{t('friends.submitAnswer')}</Button></div>}</div>)}</section>}
    {groupInvites.length > 0 && <section className="settings-section card-surface"><div className="settings-section-heading"><h2>{t('friends.groupInvites')}</h2></div>{groupInvites.map(item => <div className="friend-preview" key={item.id}><button type="button" className="profile-preview-link" disabled={!item.inviter} onClick={() => item.inviter && openProfile(item.inviter.profileKey)}><span className="friends-avatar"><span>{(displayName(item.inviter) || t('friends.member')).slice(0, 1).toUpperCase()}</span></span><span><strong>{item.groupName}</strong><small>{displayName(item.inviter) || t('friends.member')}</small></span></button><div className="friend-actions"><Button variant="secondary" onClick={() => void run(() => repository.respondGroupFriendInvitation(userId, item.id, 'accept'))}>{t('friends.acceptInvite')}</Button><button type="button" className="text-link" onClick={() => void run(() => repository.respondGroupFriendInvitation(userId, item.id, 'decline'))}>{t('friends.declineInvite')}</button></div></div>)}</section>}
