...
export function Friends({ userId, language, repository, profile, onProfile, onOpenProfile, online, onNotify }: FriendsProps) {
  ...
  return <ProfileNavigationContext.Provider value={onOpenProfile}><div className="page friends-page"><div className="page-heading"><div><span className="eyebrow">{t('friends.eyebrow')}</span><h1>{t('friends.title')}<span className="heading-period">.</span></h1><p className="muted">{t('friends.body')}</p></div><Tag tone="coral"><Icon name="lock" size={13} /> {t('common.private')}</Tag></div>
