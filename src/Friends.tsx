...
function FriendsBase({ userId, language, repository, profile, onProfile, onOpenProfile, online, onNotify }: FriendsProps) {
  ...
  return <ProfileNavigationContext.Provider value={openProfile}><div className="page friends-page"><div className="page-heading"><div><span className="eyebrow">{t('friends.eyebrow')}</span><h1>{t('friends.title')}<span className="heading-period">.</span></h1><p className="muted">{t('friends.body')}</p></div><Tag tone="coral"><Icon name="lock" size={13} /> {t('common.private')}</Tag></div>
    <section className="friends-profile card-surface"><div className="friend-code-box"><span className="eyebrow">{t('friends.code')}</span><strong>{profile.friendCode || '—'}</strong><div><button type="button" className="text-link" onClick={() => void copyCode()}>{t('friends.copyCode')}</button><button type="button" className="text-link" onClick={() => void regenerateCode()}>{t('friends.codeRegenerate')}</button></div></div></section>
