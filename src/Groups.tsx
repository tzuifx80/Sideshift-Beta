...
function GroupDetailView({ detail, language, repository, userId, onStartTeam, onBack, onNotify, onReload }: { detail: GroupDetail; language: Language; repository: AppRepository; userId: string; onStartTeam: (topic: GroupTopic | null, group: GroupSummary) => void; onBack: () => void; onNotify: (message: string) => void; onReload: () => Promise<void> }) {
  ...
  async function createInvite() {
    try { const created = await repository.createGroupInvite(userId, detail.id); setInvite(created.code); await onReload(); onNotify(t('groups.inviteCreated')) } catch (caught) { onNotify(caught instanceof Error ? caught.message : t('groups.inviteCreated')) }
  }
...
export function Groups({ userId, language, repository, onStartTeam, onBack, onNotify, initialGroupId }: GroupsProps) {
  ...
  async function create(input: CreateGroupInput) { const created = await repository.createGroup(userId, input); await reload(); const next = await repository.loadGroup(userId, created.id); setSelected(next); setShowCreate(false); onNotify(t('groups.createPrivate')) }
