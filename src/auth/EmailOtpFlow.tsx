...
import { authFlowErrorCode, type AuthFlowErrorCode } from './authFlow'
...
function errorKey(code: AuthFlowErrorCode): 'auth.emailInvalid' | 'auth.otpRequestFailed' | 'auth.otpVerificationFailed' | 'auth.rateLimited' | 'auth.expiredCode' | 'auth.emailInUse' | 'auth.sessionMissing' {
...
export function EmailOtpFlow({ language, mode, requestCode, verifyCode, onDone, onCancel, initialEmail = '' }: EmailOtpFlowProps) {
  ...
  async function sendCode() {
    ...
    try { const nextEmail = email.trim().toLowerCase(); await requestCode(nextEmail); setEmail(nextEmail); setStage('code'); setCooldown(RESEND_COOLDOWN_SECONDS) } catch (caught) { setError(t(safeErrorKey(caught, 'request'))) } finally { setBusy(false) }
  }

  async function verify() {
    setBusy(true); setError('')
    try { await verifyCode(email, code); setStage('success'); onDone?.() } catch (caught) { setError(t(safeErrorKey(caught, 'verify'))); setBusy(false) }
  }
  ...
  return <section className="auth-flow" aria-live="polite">
    ...
    {stage === 'email' ? <>
      ...
      <Button className="full-width" onClick={() => void sendCode()} disabled={busy}>{busy ? t('auth.sendingCode') : t('auth.sendCode')}</Button>
    </> : <>
      <p className="auth-flow-address">{email}</p>
      <label className="field-label" htmlFor={`auth-code-${mode}`}>{t('auth.codeLabel')}<input id={`auth-code-${mode}`} className="text-input auth-code-input" type="text" inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={code} onChange={event => { setCode(event.target.value.replace(/\D/g, '').slice(0, 6)); setError('') }} placeholder={t('auth.codePlaceholder')} /></label>
      <Button className="full-width" onClick={() => void verify()} disabled={busy || code.length !== 6}>{busy ? t('auth.verifyingCode') : t('auth.verifyCode')}</Button>
